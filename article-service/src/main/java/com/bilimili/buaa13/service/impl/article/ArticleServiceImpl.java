...
public class ArticleServiceImpl implements ArticleService {
    ...
    public Map<String, Object> getArticleWithDataById(Integer aid) {
        ...
        CompletableFuture<Void> userFuture = CompletableFuture.runAsync(() -> {
            finalMap.put("user", userArticleClient.getUserById(finalArticle.getUid()));
        }, taskExecutor);
    ...
    public ResponseResult updateArticleStatus(Integer aid, Integer status) {
        ResponseResult responseResult = new ResponseResult();
        Integer userId = userArticleClient.getCurrentUserId();
        if (status == 1 || status == 2) {
            ...
            if (userId.equals(article.getUid()) || userArticleClient.currentIsAdmin()) {
    ...
    private Map<String,Object> getArticleMap(Article article){
        ...
        map.put("user", userArticleClient.getUserById(article.getUid()));
