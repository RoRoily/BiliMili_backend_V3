...
class FavoriteControllerTest {
    ...
    void registerP() {
        ...
        responseResult.setData("registerP");
        responseResult.setMessage("登录成功");
        responseResult.setCode(500);
        HashMap<String,String> User=  new HashMap<>();
        User.put("uername","test");
        User.put("password","test");
        when(userAccountController.register(User)).thenReturn(responseResult);
        mockMvc.perform(get("/user/account/register")
                        .param(String.valueOf(User)))
